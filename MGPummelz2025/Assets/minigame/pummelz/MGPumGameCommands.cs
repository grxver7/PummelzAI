...
namespace mg.pummelz
{
    ...
    public class MGPumGameCommands
    {
        ...
        public void move(MGPumGameEvent parent, int playerID, MGPumFieldChain chain, MGPumUnit mover)
        {
            ...
            //jumping skips all other fields as they may be occupied!
            if(mover.hasMarker(MGPumMarkerAbility.Type.JumpTerrainUnits))
            {
                moveUnit(ae, mover, chain.getFirst(), chain.getLast(), true, handler);
            }
            else
            {
                ...
                foreach (MGPumField field in chain)
                {
                    ...
                    moveUnit(ae, mover, previousField, field, current == chain.getLength(), handler);
                    previousField = field;
                }
            }
        ...
        public void moveUnit(MGPumGameEvent parent, MGPumUnit mover, MGPumField originField, MGPumField targetField, bool finalField, MGPumGameEventHandler handlerToUse)
        {
            MGPumMoveUnitEvent mue = new MGPumMoveUnitEvent(mover, originField, targetField, finalField);
            mue.parent = parent;
            handlerToUse.applyEvent(mue);

            if(targetField.terrain == MGPumField.Terrain.Lava 
                && !mover.hasMarker(MGPumMarkerAbility.Type.IgnoreTerrainMoving) //flying units ignore terrain damage
                && !(mover.hasMarker(MGPumMarkerAbility.Type.JumpTerrainUnits)  && !finalField) //jumping units ignore damage of inbetween fields
                )
            {
                damageUnit(mue, targetField, mover, 1, false);
            }            
        }
        ...
        public void changeTerrain(MGPumGameEvent parent, MGPumField field, MGPumField.Terrain terrain)
        {
            MGPumChangeTerrainEvent spe = new MGPumChangeTerrainEvent(field, terrain);
            spe.parent = parent;
            handler.applyEvent(spe);
        }
