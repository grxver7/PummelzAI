    ...
    public class MGPumStateManager : MonoBehaviour
    {
        ...
        internal void visualizeNext()
        {
            ...
            if (currentEvent == null && eventsToVisualize.Count > 0)
            {
                ...
                else if (currentEvent is MGPumAttackSEvent)
                {
                    ...
                    if(MGPummelz.getColorForPlayer(me.attacker.ownerID) == MGPumConstants.RED)
                    {
                        lineAnimator.setColor(MGPumConstants.LIGHT_RED, MGPumConstants.MEDIUM_RED);
                    }
                    else
                    {
                        lineAnimator.setColor(MGPumConstants.LIGHT_BLUE, MGPumConstants.MEDIUM_BLUE);
                    }
                    ...
                    || currentEvent is MGPumChangeTerrainEvent
                    )
        ...
        internal void eventVisualized()
        {
            ...
            if (currentEvent != null)
            {
                ...
                else if (currentEvent is MGPumChangeTerrainEvent)
                {
                    MGPumChangeTerrainEvent me = (MGPumChangeTerrainEvent)currentEvent;
                    MGPumFieldRender fieldR = controller.getFieldRender(me.field);
                    fieldR.applyTerrain();
                }
