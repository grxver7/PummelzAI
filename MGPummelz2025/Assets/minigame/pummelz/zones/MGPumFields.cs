...
namespace mg.pummelz
{
    ...
    public class MGPumFields : IEnumerable<MGPumField>, MGPumZone
    {
        ...
        public MGPumFields(IDManager idm, MGPumGameConfig gc)
        {
            this.ownerID = MGPumEntity.NEUTRAL_OWNER;
            ...
            for (int x = 0; x < dimSize; x++)
            {
                for (int y = 0; y < dimSize; y++)
                {
                    ...
                    if(gc?.encounter?.terrain != null)
                    {
                        char terrainChar = gc.encounter.terrain[y][x];

                        fieldArray[x, y].terrain = gc.encounter.terrainDictionary[terrainChar];
                    }
                }
            }
        }
        ...
        public MGPumFields deepCopy(MGPumGameState state)
        {
            ...
            for (int x = 0; x < dimSize; x++)
            {
                for (int y = 0; y < dimSize; y++)
                {
                    ...
                    copy.fieldArray[x, y].terrain = this.fieldArray[x, y].terrain;
