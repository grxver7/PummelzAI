        ...
        public override bool matchesChain(MGPumField field, MGPumFieldChain chain)
        {
            ...
            else
            {
                return (
                    ...
                    //cannot attack friendly troops
                    (field.unit == null || field.unit.ownerID != startUnit.ownerID) &&
                    //cannot attack over other troops
                    (chain.last.unit == null || chain.last.unit == startUnit) &&
                    //cannot shoot over mountains (but can attack units ON mountains)
                    !(field.terrain == MGPumField.Terrain.Mountain && field.unit == null) &&
